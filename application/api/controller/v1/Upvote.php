...
class Upvote extends AuthBase {
    ...
    public function save() {
        ...
        if($userNews) {
            return show(config('code.error'), '已点赞过，不能再次点赞', [], 401);
        }
        ...
        try {
            ...
            return show(config('code.error'), $e->getMessage(), [], 401);
        }
    }
    ...
    public function delete() {
        ...
        $userNews = model('UserNews')->get($data);
        if(empty($userNews)) {
            return show(config('code.error'), '没有点赞过，不能取消', [], 401);
        }
